        ...
        <Accordion variant="splitted" className="grid outline-none lg:gap-2 px-0 h-full">
            {
                items.map(item => (
                    <AccordionItem
                        key={item.id}
                        title={<span className="block par-2 text-left px-4 p-auto">{item.title}</span>}
                        ...
                        <div className="p-4 bg-white rounded-3xl par-3 mx-4 mb-4 text-justify">{item.par}</div>
                    </AccordionItem>
                ))
            }
        </Accordion>
